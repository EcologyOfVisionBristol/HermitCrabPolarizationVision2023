...
#Script last edited by S. Smithers 25th August 2022
...
#presentation order we used a likelihood ratio test (LRT) to compare the full model with the same model but 
#with the effect of interest removed.
...
#Load raw data
data<-read.csv(here::here("data","csv","20200218_PurpleHermit_(C.brevimanus)_P_response.csv"),header=T)
data <-na.omit(data) # Remove any rows containing NA 
...
rm(list = ls())
#Load raw data
data<-read.csv(here::here("data","csv","20200221_PurpleHermit_(C.brevimanus)_I_response.csv"),header=T)
...
rm(list = ls())
#Load raw data
data<-read.csv(here::here("data","csv","20200212_PaleHermit_(C.rugosus)_P_response.csv"),header=T)
data <-na.omit(data)#Remove any rows contain NA 
data$Binary_response <-factor(data$Binary_response) #set binary response to a factor
data$Crab_ID<-factor(data$Crab_ID) # Set crab ID to a factor
...
rm(list = ls())
#Load raw data
data<-read.csv(here::here("data","csv","20200216_PaleHermit_(C.rugosus)_I_response.csv"),header=T)
data <-na.omit(data)#Remove any rows contain NA 
data$Binary_response <-factor(data$Binary_response) #set binary response to a factor
data$Crab_ID<-factor(data$Crab_ID) # Set crab ID to a factor
...
rm(list = ls())
#Load raw data
data<-read.csv(here::here("data","csv","Pagurus_P_response.csv"),header=T)
data <-na.omit(data)#Remove any rows contain NA 
data$Binary_response <-factor(data$Binary_response) #set binary response to a factor
data$Crab_ID<-factor(data$Crab_ID) # Set crab ID to a factor
